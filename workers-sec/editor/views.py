...
HEARTBEAT_MISS_TIMEOUT = 8*HEARTBEAT_TIMEOUT	# Time after which missing heartbeats is considered failure
...
mon=threading.Thread(target=moni)
mon.start()

def sec_maker():
	global STATE
	while(1):
		# print "sec maker running"
		if time.time()-LAST_LEASE_RECEIVED>LEASE_TIMEOUT:
			STATE='secondary'
		time.sleep(0.2)
...
def lease_new(request):
	global STATE
	global LAST_LEASE_RECEIVED
	print "in lease new with request.method ", request.method
	if request.method=='GET':
		...
		while (not done):
			try:
				r2=requests.get(MASTER_URL+'/api/lease_extend_ack/')
				done=True
			except:
				pass
	return JsonResponse({'ok':'ok'})  

def lease_extend(request):
	global LAST_LEASE_RECEIVED
	LAST_LEASE_RECEIVED=time.time()
	print "in lease_extend"
	time.sleep(1)
	if request.method=='GET':
		# STATE='primary'
		# LAST_LEASE_RECEIVED=time.time()
...
def index(request, document_id=None):
	if(not request.GET.get('from-master')):
		return HttpResponse('Go, you stupid idiot, to '+MASTER_URL)
	if STATE == 'primary':	
		if not document_id:
			document_id = 'default'
		...
		return HttpResponse('Go, you mf idiot, to '+MASTER_URL)
